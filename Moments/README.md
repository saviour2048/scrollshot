...
- 浮动「+」快速记录：文字 + 照片/视频（PhotosPicker）+ 语音录制 + 心情 + 地点 + 标签，自动时间戳
- 心情图标（😄🙂😐😔😣）显示在卡片与详情
- 地点：CoreLocation 一次性定位 + 反查地名，详情用 MapKit 小地图展示、可跳系统地图
- 「去年今日」横幅：往年同月同日的记录，点开按年份回看
- 标签筛选 + 标签管理页（改名 / 改色 / 合并 / 删除）
- 详情：大图、视频播放（AVKit）、语音播放条；编辑、删除
- 首次启动预置「想法 / 生活 / 工作 / 心情」标签

> 桌面 Widget 留到后续（需 App Group 共享存储）。
